...
//                .join();
    ...
    public static String httpGetRequest(String idFromPost, String itemCodeFromPost){
        ...
        //The string that will be returned, not sure why it needs to an array for it to be assigned a value below
        final String[] requestResult = new String[1];

        client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(HttpResponse::body)
                //.thenApply(HttpRequest::parseGetRequest)
                .thenAccept(s -> requestResult[0] = s)
                .join();
    ...
    public static String parseGetRequest(String responseBody){
        ...
        result = result1.getJSONObject(0);
        //Find the location of where the price of the item is stored
        result = result.getJSONObject("listing").getJSONObject("price");
        //Putting the price into a readable format as follows "number currency type" e.g. "0.1 chaos"
        String price = result.getDouble("amount") + " " + result.getString("currency");
        return price;
    }
    ...
    //the first element of this arraylist is the id, the rest are the
